...
var input = File.ReadAllLines(Path.Combine(Path.GetDirectoryName(Util.CurrentQueryPath), "day17.input.txt"))
...
var (xMin, xMax, yMin, yMax) = input.Aggregate(
...
var dumper = new DumpContainer().Dump();
void DumpMap() => dumper.UpdateContent(
	string.Join(Environment.NewLine,
		ground.Select(s => string.Join("", s.AsEnumerable())).Concat(new[] { "" })));
...
(tileTypes['~'] + tileTypes['|']).Dump("Part A");
tileTypes['~'].Dump("Part B");
