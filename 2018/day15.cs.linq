...
void DumpMap() =>
	string.Join(Environment.NewLine,
		space.Select(s => string.Join("", s.AsEnumerable())).Concat(new[] { ""})).Dump();

void Main()
{
	input = File.ReadAllLines(Path.Combine(Path.GetDirectoryName(Util.CurrentQueryPath), "day15.input.txt"));
	RunGame(true, 3).Dump("Part A");
