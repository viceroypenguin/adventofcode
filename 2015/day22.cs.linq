...
Tuple<int, ImmutableStack<TurnAction>> HandleTurn(CharacterSet set)
{
	return _HandleTurn(set, Tuple.Create(int.MaxValue, ImmutableStack<TurnAction>.Empty), Tuple.Create(0, ImmutableStack<TurnAction>.Empty));
}

Tuple<int, ImmutableStack<TurnAction>> _HandleTurn(CharacterSet set, Tuple<int, ImmutableStack<TurnAction>> minActions, Tuple<int, ImmutableStack<TurnAction>> actionsSoFar)
{
	set.DoNextTurn();
	...
	switch (set.WhoseTurn)
	{
		case WhoseTurn.BossTurn:
			...
			return _HandleTurn(set, minActions, Tuple.Create(actionsSoFar.Item1, actionsSoFar.Item2.Push(TurnAction.BossAttack)));
		...
		case WhoseTurn.PlayerTurn:
			foreach (var effect in _Effects)
			{
				...
				if (testCost > minActions.Item1)
					continue;
				var playCost = _HandleTurn(testSet, minActions, Tuple.Create(testCost, actionsSoFar.Item2.Push(effect.TurnAction)));
...
class CharacterSet
{
	...
	public void DoNextTurn()
	{
		...
		if (part2 && WhoseTurn == WhoseTurn.PlayerTurn)
			Player.HitPoints--;
	}
